...
def render_p1_table(surge_table, rsi_snapshot, watchlist=None, market_filter="전체"):
    ...
    def _name_color(row):
        ...
        if mkt == "KOSPI":
            styles[idx] = "color:#1565C0;font-weight:700"
        elif mkt == "KOSDAQ":
            styles[idx] = "color:#E65100;font-weight:700"
        return styles
    ...
    def _chg_colors(col_series):
        ...
        return [
            f"color:{'#C62828' if v>0 else '#1565C0' if v<0 else '#333'};font-weight:700"
            for v in raw
        ]
    ...
    if "현재가"  in df_show.columns:
        fmt["현재가"]  = lambda v: f"{int(v):,}원" if v else "-"
    if "당일등락" in df_show.columns:
